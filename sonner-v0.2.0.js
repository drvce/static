...
(() => {
    ...
    const ENTER_ANIMATION_MS = 140;
    const EXIT_ANIMATION_MS = 110;
...
[data-sonner-vanilla-toast] {
  ...
  opacity: 0;
  transform: translateY(var(--sonner-enter-y, 0));
  animation: sonner-vanilla-enter ${ENTER_ANIMATION_MS}ms cubic-bezier(0.16, 1, 0.3, 1) forwards;
  will-change: opacity, transform;
}

[data-sonner-vanilla-root][data-position^="top-"] [data-sonner-vanilla-toast] {
  --sonner-enter-y: -8px;
}

[data-sonner-vanilla-root][data-position^="bottom-"] [data-sonner-vanilla-toast] {
  --sonner-enter-y: 8px;
}

[data-sonner-vanilla-toast][data-state="closing"] {
  pointer-events: none;
  animation: sonner-vanilla-exit ${EXIT_ANIMATION_MS}ms cubic-bezier(0.4, 0, 1, 1) forwards;
}
...
@keyframes sonner-vanilla-enter {
  from {
    opacity: 0;
    transform: translateY(var(--sonner-enter-y, 0));
  }
...
`;
    ...
    function createToaster(initialOptions) {
        ...
        function clearExitTimer(handle) {
            if (handle.exitTimerId !== null) {
                window.clearTimeout(handle.exitTimerId);
                handle.exitTimerId = null;
            }
        }
        function finalizeRemoveHandle(handle, reason) {
            var _a, _b, _c, _d;
            clearTimer(handle);
